...
<div class="small-container">
    <div class="row row-2">
        <h2>Result for <?php echo $title ?></h2>
        <select onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
            <option value="?control=category&id=<?php echo $id ?>&field=default">Default Sorting</option>
            <option <?php if ($orderField == 'name' && $orderSort == 'asc') { ?> selected <?php } ?> value="?control=search&kw=<?php echo $keyword?>&field=name&sort=asc">
                Name: A-Z
            </option>
            <option <?php if ($orderField == 'name' && $orderSort == 'desc') { ?> selected <?php } ?> value="?control=search&kw=<?php echo $keyword?>&field=name&sort=desc">
                Name: Z-A
            </option>
            <option <?php if ($orderField == 'price' && $orderSort == 'asc') { ?> selected <?php } ?> value="?control=search&kw=<?php echo $keyword?>&field=price&sort=asc">
                Price: Low-High
            </option>
            <option <?php if ($orderField == 'price' && $orderSort == 'desc') { ?> selected <?php } ?> value="?control=search&kw=<?php echo $keyword?>&field=price&sort=desc">
                Price: High-Low
            </option>
        </select>
    </div>
    ...
    $sql = "SELECT * FROM product WHERE product_name LIKE '%$keyword%' AND product_active = 1 ".$orderCondition." LIMIT $offset, $productPerPage";
    ...
    $lpm1 = $maxPage - 1;
    $self = "?control=search&kw=$keyword".$param;
    $nav  = '';
    ?>
